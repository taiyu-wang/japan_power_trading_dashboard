...
def complete_half_hourly_days(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df.copy()
    keys = ["area", "date", "period_id"]
    work = df.sort_values(keys).drop_duplicates(keys, keep="last").copy()
    coverage = (
        work.groupby(["area", "date"], as_index=False)["period_id"]
        .agg(period_count="nunique", first_period="min", last_period="max")
    )
    valid = coverage[
        coverage["period_count"].eq(48)
        & coverage["first_period"].eq(1)
        & coverage["last_period"].eq(48)
    ][["area", "date"]]
    return work.merge(valid, on=["area", "date"], how="inner").sort_values(
        ["area", "datetime"] if "datetime" in work.columns else keys
