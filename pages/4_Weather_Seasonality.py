...
from src.utils import configure_page, download_button, live_fetch_spinner, page_header, render_chart, sample_data_notice
...
with live_fetch_spinner("Refreshing Open-Meteo daily temperatures...", use_live_weather):
...
with weather_tab:
    ...
    if weather.empty:
        ...
        render_chart(line_chart(weather, "date", "temperature_mean_c", "region", "Tokyo/Kansai Daily Temperature", "deg C"))
        ...
        render_chart(line_chart(degree_pressure, "date", "degree_day_pressure", "region", "Tokyo/Kansai Degree-Day Pressure", "CDD + HDD"))

        if not joined.empty:
            left, right = st.columns(2)
            with left:
                render_chart(temperature_power_chart(joined, "Tokyo", "Tokyo Temperature vs JEPX Tokyo"))
            with right:
                render_chart(temperature_power_chart(joined, "Kansai", "Kansai Temperature vs JEPX Kansai"))
        ...
        with st.expander("Regional spread and degree-day totals", expanded=False):
            ...
            if {"Tokyo", "Kansai"}.issubset(wide.columns):
                ...
                render_chart(line_chart(spread, "date", "value", "market", "Tokyo-Kansai Temperature Spread", "deg C"))
            ...
            render_chart(bar_chart(degree_long, "region", "value", "metric", "Cumulative Degree-Day Load"))
...
with seasonality_tab:
    ...
    if subset.empty:
        ...
        render_chart(line_chart(monthly, "month_name", "price", "year", f"{market} Monthly Seasonal Profile"))

        weekly = subset.groupby(["year", "week"], as_index=False)["price"].mean()
        render_chart(line_chart(weekly, "week", "price", "year", "Weekly Seasonal Track"))
        ...
        render_chart(bar_chart(pd.concat([summer, winter]), "year", "price", "season", "Peak Summer / Winter Comparison"))
