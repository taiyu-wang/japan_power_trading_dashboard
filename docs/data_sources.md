...
|---|---|---:|---|---:|---|---|
| Historical fuel/power/FX prices | `data/sample_historical_prices.csv` | Synthetic sample | `PYTHONPATH=. python -m src.sample_data` | Low | No | Licensed JEPX, Platts/Argus, ICE/CME, broker, customs, FX feed |
...
| JEPX intraday liquidity/convergence | `data/processed/jepx_intraday_latest.csv` | Compact public processed data | Existing processed cache | Medium for ex-post liquidity | No | Direct JEPX public refresh pipeline or vendor history |
| JEPX physical baseload auction tracker | `data/processed/jepx_baseload_market.csv` | Compact public processed data | Existing processed cache | Medium for ex-post auction context | No | JEPX public auction refresh pipeline or vendor history |
| JEPX day-ahead offer stack / bidding curves | `data/processed/jepx_offer_stack_depth.csv`; `data/processed/jepx_offer_stack_curves_compact.csv` | Public ex-post processed data | `PYTHONPATH=. python -m src.offer_stack` | Medium for ex-post market structure | No | Exchange/member access, vendor order-book/participant/unit-level data |
...
| Weather | `data/sample_weather_temperatures.csv`; optional Open-Meteo refresh | Synthetic sample plus public weather option | Bundled refresh or page toggle | Medium for public weather screening | No | JMA, ECMWF, DTN, Meteomatics, vendor forecasts/history |
| Tokyo/Kansai generation mix | `data/sample_generation_mix.csv`; processed public-looking aggregates | Synthetic fallback / compact processed | Bundled refresh or upload | Low unless uploaded/validated | No by default | OCCTO/TSO/METI reconciled data, vendor generation by fuel/area |
| Power news | `data/sample_power_news.csv`; optional public refresh | Sample plus public RSS/HTML | Bundled refresh or page toggle | Medium for public context; low latency not guaranteed | No by default | Paid news/outage/vendor feeds, broker notes, internal event database |
...
- Public power news from JEPX/OCCTO/public RSS sources.
- JEPX day-ahead public bidding-curve download through `src.offer_stack`.
...
- **Supply mix:** Public regional fuel-type splits are fragmented. The bundled Tokyo/Kansai mix is synthetic unless replaced with validated public/vendor/internal data.
...
4. Build a scheduled public JEPX ingestion job for spot, intraday, baseload, and offer-stack datasets.
5. Replace supply mix with reconciled OCCTO/TSO/METI/vendor area/fuel data.
